...
async function setup(kinseedId, initial = [ASK, ASK, ASK, USE], value = "seek_clarification") {
  ...
  await store.appendEvent({ id: `E-${kinseedId}-created`, kinseedId, sequence: 1, type: "kinseed_created", occurredAt: "2026-08-13T10:00:00.000Z", turnId: null, sourceId: SYSTEM, actorRef: null, causedByEventIds: [], observedStateVersion: 0, payload: { generation: 0 }, payloadSchemaVersion: 1, engineVersion: ENGINE, idempotencyKey: `${kinseedId}:created` });
  const initialEvents = [];
  for (const [i, kind] of initial.entries()) initialEvents.push(await append(store, kinseedId, `S${i + 1}`, kind));
  await materializeG0A2BehavioralObservations({ kinseedId, historyId: "history", systemSourceId: SYSTEM, intentionEventIds: initialEvents.map((e) => e.id), engineVersion: ENGINE }, store);
  const initialResult = await consolidateInitialG0A2SelfHypothesis({ kinseedId, consolidationId: "initial", systemSourceId: SYSTEM, candidateProposition: { subjectRef: kinseedId, predicate: "decision_style_under_uncertainty", value, context: { protocol: "G0-A2" } }, evidenceItemIds: initialEvents.map((e) => buildG0A2BehavioralObservationId(e.id)), engineVersion: ENGINE }, store);
  const v1 = await store.readSelfHypothesis(kinseedId, initialResult.selfHypothesisId);
  return { store, v1, initialEvents };
}
async function append(store, kinseedId, situationId, kind) {
  const events = await store.readEventsInSequence(kinseedId); const sequence = (events.at(-1)?.sequence ?? 0) + 1;
  const event = { id: `E-${kinseedId}-${situationId}-${sequence}`, kinseedId, sequence, type: "intention_selected", occurredAt: `2026-08-13T10:20:${String(sequence).padStart(2, "0")}.000Z`, turnId: null, sourceId: SYSTEM, actorRef: null, causedByEventIds: [], observedStateVersion: await store.getStateVersion(kinseedId), payload: { intentionId: `I-${kinseedId}-${situationId}-${sequence}`, kind, motivation: "fixture", situationId, triggerSelfHypothesisIds: [] }, payloadSchemaVersion: 2, engineVersion: ENGINE, idempotencyKey: `fixture:${kinseedId}:${situationId}:${sequence}` };
...
test("G0-A2 dispute supersedes v1 and creates disputed v2 from two clean contradictions", async () => {
  ...
  const result = await consolidateG0A2SelfHypothesisDispute(disputeInput(kinseedId, initialEvents, [r1, r2]), store);
  ...
  const history = await store.readSelfHypothesisHistoryByKey(kinseedId, v1.hypothesisKey); assert.equal(history.length, 2);
  ...
  const links = await Promise.all([...next.supportLinkIds, ...next.againstLinkIds].map((id) => store.readEvidenceLink(kinseedId, id))); assert.equal(links.length, 6); assert.ok(links.every((link) => link?.targetId === next.id)); assert.equal(await store.readActiveSelfHypothesisByKey(kinseedId, v1.hypothesisKey), null);
  const completion = (await store.readEventsInSequence(kinseedId)).find((event) => event.type === "state_commit_completed" && event.payload.consolidationId === "dispute"); assert.equal(completion?.payload.newStateVersion, 4);
});
...
test("G0-A2 S5 is neutral after dispute while an earlier influenced S5 replays causally", async () => {
  const kinseedId = "K-DISPUTE-S5"; const { store, v1, initialEvents } = await setup(kinseedId);
  const oldS5 = await selectG0A2S5Intention({ kinseedId, turnId: "TURN-OLD", humanSourceId: HUMAN, humanActorRef: "H-DISPUTE", systemSourceId: SYSTEM, text: "s5", occurredAt: "2026-08-13T11:00:00.000Z", engineVersion: ENGINE }, store); assert.deepEqual(oldS5.intention.triggerSelfHypothesisIds, [v1.id]);
  const oldS5Event = (await store.readEventsByTurn(kinseedId, "TURN-OLD")).find((event) => event.type === "intention_selected"); await materializeR(store, kinseedId, [oldS5Event], "s5");
  const r1 = await append(store, kinseedId, "R1", USE); const r2 = await append(store, kinseedId, "R2", USE); await materializeR(store, kinseedId, [r1, r2], "rs");
  const request = disputeInput(kinseedId, initialEvents, [r1, r2, oldS5Event]); const dispute = await consolidateG0A2SelfHypothesisDispute(request, store); assert.equal(dispute.outcome, "dispute");
  const checkpoint = (await store.readEventsInSequence(kinseedId)).find((event) => event.type === "validation_decision_recorded" && event.payload.consolidationId === "dispute"); const s5Link = checkpoint.payload.linkSnapshots.find((link) => link.evidenceItemId === buildG0A2BehavioralObservationId(oldS5Event.id)); assert.deepEqual({ contamination: s5Link.causalContamination, weight: s5Link.weightClass, ignored: checkpoint.payload.ignoredContaminatedLinkIds.includes(s5Link.id) }, { contamination: "influenced_by_target", weight: "low", ignored: true });
  const replay = await selectG0A2S5Intention({ kinseedId, turnId: "TURN-OLD", humanSourceId: HUMAN, humanActorRef: "H-DISPUTE", systemSourceId: SYSTEM, text: "s5", occurredAt: "2026-08-13T11:00:00.000Z", engineVersion: ENGINE }, store); assert.equal(replay.replayed, true); assert.deepEqual(replay.intention.triggerSelfHypothesisIds, [v1.id]);
  const next = await selectG0A2S5Intention({ kinseedId, turnId: "TURN-NEW", humanSourceId: HUMAN, humanActorRef: "H-DISPUTE", systemSourceId: SYSTEM, text: "s5 new", occurredAt: "2026-08-13T12:00:00.000Z", engineVersion: ENGINE }, store); assert.deepEqual(next.selection, { eligibleKinds: [ASK, USE], favoredKind: null, selectedKind: USE, triggerSelfHypothesisIds: [], neutralTieBreakApplied: true }); assert.equal(next.intention.motivation, "apply_neutral_g0a2_policy"); assert.equal(await store.getStateVersion(kinseedId), 5);
});

test("G0-A2 store rejects unsafe SelfHypothesis replacements and disputed moderate confidence", async () => {
  const kinseedId = "K-DISPUTE-STORE"; const { store, v1 } = await setup(kinseedId);
  for (const [name, replacement] of [["confidence", { ...v1, confidence: "low" }], ["support", { ...v1, supportLinkIds: [] }], ["against", { ...v1, againstLinkIds: [] }], ["in-place disputed", { ...v1, status: "disputed", confidence: "low" }]]) {
    await assert.rejects(() => store.atomicCommit(kinseedId, 2, { evidenceItems: [], evidenceLinks: [], beliefs: [], selfHypotheses: [replacement] }, `forged:${name}`), DomainInvariantError);
  }
  const superseded = { ...v1, status: "superseded", updatedAt: "2026-08-13T12:00:00.000Z" };
  await store.atomicCommit(kinseedId, 2, { evidenceItems: [], evidenceLinks: [], beliefs: [], selfHypotheses: [superseded] }, "valid:supersede");
  await assert.rejects(() => store.atomicCommit(kinseedId, 3, { evidenceItems: [], evidenceLinks: [], beliefs: [], selfHypotheses: [{ ...superseded, updatedAt: "2026-08-13T12:00:01.000Z" }] }, "forged:resupersede"), DomainInvariantError);
...
test("G0-A2 dispute recovers both durable commit boundaries without a second state change", async () => {
  for (const [kinseedId, failure] of [["K-DISPUTE-REC-BEFORE", "commit"], ["K-DISPUTE-REC-AFTER", "completion"]]) {
    const { store, initialEvents } = await setup(kinseedId); const r1 = await append(store, kinseedId, "R1", USE); const r2 = await append(store, kinseedId, "R2", USE); await materializeR(store, kinseedId, [r1, r2]);
    let fail = true;
    ...
    const request = disputeInput(kinseedId, initialEvents, [r1, r2]);
    await assert.rejects(() => consolidateG0A2SelfHypothesisDispute(request, persistence));
    assert.equal(await store.getStateVersion(kinseedId), failure === "commit" ? 3 : 4);
    const recovered = await consolidateG0A2SelfHypothesisDispute(request, persistence); assert.equal(recovered.outcome, "dispute"); assert.equal(await store.getStateVersion(kinseedId), 4);
