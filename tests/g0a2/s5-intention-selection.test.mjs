...
async function setupHistory(lenoSeedId, kinds, candidateValue) {
  ...
  await materializeG0A2BehavioralObservations({ lenoSeedId, historyId: "history", systemSourceId, intentionEventIds: fixtures.map((event) => event.id), engineVersion }, store);
  const result = await consolidateInitialG0A2SelfHypothesis({ lenoSeedId, consolidationId: "initial", systemSourceId, candidateProposition: { subjectRef: lenoSeedId, predicate: "decision_style_under_uncertainty", value: candidateValue, context: { protocol: "G0-A2" } }, evidenceItemIds: fixtures.map((event) => buildG0A2BehavioralObservationId(event.id)), engineVersion }, store);
...
function input(lenoSeedId, turnId = `TURN-${lenoSeedId}`, text = s5Text) {
  return { lenoSeedId, turnId, humanSourceId, humanActorRef, systemSourceId, text, occurredAt: "2026-08-13T10:01:00.000Z", engineVersion };
}
  ...
  return new Proxy(store, {
    get(target, property, receiver) {
      if (property === "readEventsByTurn") {
        return async (lenoSeedId, turnId) => mutate(await target.readEventsByTurn(lenoSeedId, turnId));
      }
...
test("G0-A2 S5 selects divergent structured intentions for histories A and B", async () => {
  ...
  for (const [store, result] of [[historyA.store, selectedA], [historyB.store, selectedB]]) {
    const events = await store.readEventsByTurn(result.intention.lenoSeedId, result.situationEvent.turnId);
    ...
    assert.equal(await store.getStateVersion(result.intention.lenoSeedId), 2);
  }
});
...
test("G0-A2 S5 replays the historical intention without reading the current active hypothesis", async () => {
  ...
  const eventCount = (await store.readEventsInSequence(request.lenoSeedId)).length;
  ...
  assert.equal((await store.readEventsInSequence(request.lenoSeedId)).length, eventCount);
  assert.equal(await store.getStateVersion(request.lenoSeedId), 2);
});
...
test("G0-A2 S5 replay preserves its historical state version after a later state", async () => {
  ...
  assert.equal(
    (await store.readEventsInSequence(request.lenoSeedId)).length,
    eventCount,
  );
});
...
test("G0-A2 S5 rejects replay and historical incoherence", async (t) => {
  ...
  await t.test("malformed existing S5 input", async () => {
    ...
    await store.appendEvent({ id: `E-${request.turnId}-input`, lenoSeedId: request.lenoSeedId, sequence: 9, type: "human_message_received", occurredAt: request.occurredAt, turnId: request.turnId, sourceId: humanSourceId, actorRef: humanActorRef, causedByEventIds: [], observedStateVersion: 2, payload: { text: request.text }, payloadSchemaVersion: 1, engineVersion, idempotencyKey: `${request.turnId}:input` });
    await assert.rejects(() => selectG0A2S5Intention(request, store), DomainInvariantError);
  });
...
test("G0-A2 S5 retry after intention append failure reuses its input event", async () => {
  ...
  assert.equal((await store.readEventsByTurn(request.lenoSeedId, request.turnId)).filter((event) => event.type === "human_message_received").length, 1);
  const retried = await selectG0A2S5Intention(request, flaky);
  assert.equal(retried.replayed, false);
  assert.equal((await store.readEventsByTurn(request.lenoSeedId, request.turnId)).length, 2);
  assert.equal(await store.getStateVersion(request.lenoSeedId), 2);
});
