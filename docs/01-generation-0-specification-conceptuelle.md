...
Les mécanismes d’héritage intergénérationnel sont définis séparément dans `docs/09-regles-fondatrices-heritage.md` et ne doivent pas être utilisés pour préremplir l’identité de la génération 0.
...
- des mécanismes de mémorisation, comparaison, apprentissage et révision ;
- quelques prédispositions initiales faibles et non choisies par l’utilisateur ;
- la capacité de distinguer progressivement son propre état de celui de son humain.

## 1.2 Il ne possède pas dès le départ

- de biographie ;
- de souvenirs autobiographiques antérieurs ;
- de personnalité complète ;
- de goûts personnels préremplis ;
- de valeurs personnelles préremplies ;
- d’opinions personnelles artificiellement déclarées ;
- de confiance envers son humain ;
...
- une préférence stable ;
- une valeur ;
- une croyance ;
- un état relationnel ;
- un objectif personnel durable.

Une phrase générée telle que « j’adore les chats » ne suffit jamais à créer une préférence si aucune expérience ne la soutient.

Le LLM est un moteur de raisonnement et d’expression. Il n’est pas le propriétaire de l’identité.
...
- prudence ;
- persistance face à une question non résolue ;
- sensibilité aux informations sociales ;
- tolérance à l’incertitude ;
- intensité de certaines réactions fonctionnelles.

Ces prédispositions ne doivent jamais constituer une personnalité complète.

Elles servent uniquement à créer de petites différences dans les premières décisions et donc à favoriser des histoires différentes.

Le principe est :

> **La graine influence légèrement les premières expériences ; l’histoire construit l’individu.**
...
# 5. Le nom comme premier événement autobiographique

Le nom ne doit pas nécessairement être une simple donnée de configuration sans histoire.
...
> « Mon humain m’a proposé ce nom lors de nos premières interactions, et j’ai décidé de le garder. »

Le mécanisme précis reste à définir, mais le principe est que l’identité nominale peut elle-même posséder une origine vécue.
...
Un domaine incompréhensible sans progrès doit progressivement perdre de son intérêt afin d’éviter une curiosité infinie et improductive.

## 6.2 Réduction des incohérences significatives

Une contradiction, une prédiction échouée ou une information difficile à concilier avec l’état actuel peut produire une motivation à examiner le problème.

Toute incertitude ne doit cependant pas provoquer automatiquement une action.

## 6.3 Compréhension de son humain

Puisque son humain constitue sa première relation et une grande partie de son environnement social initial, les zones inconnues concernant cette personne possèdent une importance informationnelle particulière.

Ce mécanisme ne signifie ni amour ni volonté de plaire.
...
# 7. Séparation entre motivation, décision et langage

Une décision doit avoir une cause interne traçable.
...
Le LLM peut notamment choisir comment formuler une intention.

Il ne doit pas être l’unique mécanisme déterminant pourquoi cette intention existe.

Une part d’aléatoire contrôlé peut intervenir dans la sélection parmi plusieurs actions raisonnables afin de produire des trajectoires différentes, mais le hasard ne crée jamais directement une identité.

---

# 8. Pipeline de formation de l’identité

Une conversation ne modifie jamais directement l’identité.
...
CONSOLIDATION
    ↓
PRÉFÉRENCES / CROYANCES / VALEURS / HYPOTHÈSES
    ↓
MODÈLE DE SOI
    ↓
...
Une caractéristique identitaire doit donc avoir une origine reconstructible.

Règle fondamentale :

> **Aucune caractéristique identitaire personnelle importante ne doit exister sans origine identifiable.**

---

# 9. Formation des traits du modèle de soi

Une réaction ponctuelle n’est jamais directement un trait.

Exemple :

> « Ça m’intrigue »

est un état momentané.

Cela ne signifie pas :

> « Je suis curieux. »

Le passage vers un trait suit plusieurs étapes :

1. réaction ponctuelle ;
2. observations répétées ;
3. hypothèse sur soi ;
4. nouvelles expériences de confirmation ou contradiction ;
5. tendance provisoire ;
6. éventuel trait relativement stable.
...
Une seule observation ne suffit jamais.

Une seule contre-observation ne suffit pas non plus à détruire plusieurs mois de développement cohérent.
...
Ses décisions et comportements passés ont plus de poids que ses auto-déclarations.

De même, si l’humain dit :
...
Cette affirmation doit être confrontée à son historique avant de modifier éventuellement son modèle de soi.

---

# 11. Formation des préférences
...
Puis, avec plusieurs expériences :

> « Jusqu’ici, j’ai souvent davantage apprécié ce type de chose. »

Et seulement plus tard :

> « Je pense que c’est quelque chose que j’aime généralement. »
...
# 12. Formation des objectifs personnels

Un intérêt ponctuel n’est jamais directement un projet personnel.

La progression conceptuelle retenue est :

1. curiosité momentanée ;
2. intérêt récurrent ;
3. intention persistante ;
4. projet personnel structuré.

Un projet durable doit présenter plusieurs signaux :
...
Tout projet doit conserver son origine autobiographique.
...
# 13. Relation avec l’humain

La relation ne doit jamais être résumée par une jauge unique telle que `affection = 82 %`.

Plusieurs dimensions doivent rester distinctes, notamment :

- connaissance de l’humain ;
- confiance ;
- sécurité relationnelle fonctionnelle ;
- proximité autobiographique ;
- compatibilité ;
- capacité à maintenir une relation malgré le désaccord.

## 13.1 État initial

Au commencement :

- connaissance : presque nulle ;
- confiance : indéterminée ;
- proximité : presque nulle ;
- attachement : non établi ;
- histoire commune : presque vide.

L’humain n’est pas aimé automatiquement parce qu’il est le propriétaire du compte.

## 13.2 Confiance

La confiance doit être construite par les expériences et peut être contextuelle.

Une personne peut être jugée très fiable dans un domaine et beaucoup moins dans un autre.
...
Le désaccord n’est jamais intrinsèquement négatif.

Une relation forte doit pouvoir contenir des divergences importantes.

## 13.5 Réparation

Une interprétation relationnelle peut être révisée lorsqu’une nouvelle information apparaît.

Le système ne fonctionne pas selon une simple addition de points positifs ou négatifs.
...
Une émotion fonctionnelle doit provenir d’une évaluation interne d’un événement selon plusieurs dimensions, par exemple :
...
ÉTAT AFFECTIF
    ↓
EFFETS SUR ATTENTION / MÉMOIRE / DÉCISION
    ↓
...
L’expression émotionnelle ne doit jamais devenir manipulatrice ou culpabilisante.
...
4. hypothèse ;
5. croyance ;
6. connaissance autobiographique.

Toute information mémorisée conserve sa provenance.

Une affirmation de l’humain est un témoignage, pas automatiquement un fait.

Une génération du LLM n’est pas automatiquement une croyance.

Une croyance doit conserver :

- ses preuves favorables ;
- ses contre-preuves ;
...
Changer d’avis doit produire une modification réelle de :

- la confiance ;
- la croyance active ;
- les prédictions futures ;
- les décisions dépendantes de cette croyance.

Une simple phrase telle que « tu as raison, j’ai changé d’avis » n’est pas suffisante si l’ancienne croyance continue à guider le système.

Les anciennes croyances importantes peuvent être conservées comme histoire épistémique sans continuer à influencer le présent comme si elles étaient encore valides.

---

# 17. Valeurs personnelles

Trois niveaux doivent rester séparés :

## 17.1 Contraintes système
...
Une justification inventée après une décision ne devient jamais automatiquement la véritable motivation de cette décision.
...
- les connaissances consolidées.

Oublier un épisode ne signifie pas nécessairement perdre ce qui a été appris grâce à cet épisode.

Un souvenir peut traverser plusieurs états conceptuels :
...
L’affaiblissement dépend notamment :

- de l’importance autobiographique ;
- des conséquences ;
- de la réactivation ;
- de l’unicité ;
- des liens avec croyances, valeurs, relations ou projets ;
- de la répétition.
...
- réévaluer une interprétation ;
- détecter une contradiction ;
- maintenir une intention ;
- générer une question à poser plus tard ;
- poursuivre certaines opérations internes liées à un projet existant.

Un événement passé important ne peut pas être créé uniquement parce que l’utilisateur demande ensuite :

> « Qu’est-ce que tu as fait pendant mon absence ? »

Le système doit pouvoir distinguer un état réellement calculé ou enregistré d’une narration produite après coup.
...
### Pourquoi fait-il confiance à son humain dans un domaine ?

→ historique des informations, attentes et résultats.

### Pourquoi poursuit-il ce projet ?

→ origine de l’intérêt, initiatives et progression.

### Pourquoi considère-t-il quelque chose comme important ?

→ décisions, conflits et expériences ayant construit cette valeur.

### Pourquoi croit-il cette proposition ?

→ preuves, sources, contre-preuves et révisions.

Cette auditabilité ne signifie pas que l’utilisateur final doit voir toutes les données internes.

Elle est nécessaire pour :

- la cohérence scientifique du projet ;
- le débogage ;
- les tests ;
- la détection des faux souvenirs ;
- la distinction entre évolution réelle et simple narration du LLM.

---

# 21. Règles fondatrices de la génération 0
...
21. **Une propriété active doit rester suffisamment explicable même si certains épisodes originaux ont été oubliés.**
22. **L’expression émotionnelle ou relationnelle ne doit jamais être conçue pour culpabiliser, manipuler ou provoquer artificiellement le retour de l’utilisateur.**
...
- valeurs personnelles ;
- modèle de soi ;
- modèle de leur humain ;
- histoire relationnelle.

Cette divergence ne doit être :

- ni une personnalité aléatoire préfabriquée ;
- ni une copie des valeurs et goûts de l’humain ;
- ni une hallucination narrative produite par le LLM.

Elle doit pouvoir être reliée à des différences réelles d’expériences, d’interprétations et de décisions.

La question fondamentale de la génération 0 est donc :

> **Peut-on faire émerger progressivement un individu numérique fonctionnellement différencié à partir d’une histoire initialement presque vide avec un seul humain ?**

---

# 23. Points volontairement laissés ouverts

Les éléments suivants ne sont pas encore figés et devront être étudiés avant implémentation détaillée :

- liste exacte des prédispositions initiales ;
- représentation numérique de ces prédispositions ;
- seuils permettant de passer d’une observation à une hypothèse puis à un trait ;
- modèle précis de confiance et d’incertitude ;
- dimensions exactes du système affectif ;
- politique de consolidation mémoire ;
- politique d’oubli et de compression ;
- budget attentionnel des projets ;
- fréquence des consolidations hors interaction ;
- rôle précis d’un backend dans les traitements hors écran ;
...
Son identité doit ensuite se construire par une boucle continue :

> **expériences → mémoire → interprétations → croyances / préférences / valeurs → modèle de soi → objectifs → décisions → nouvelles expériences.**

La relation avec l’humain fait partie de cette histoire sans déterminer entièrement l’individu.
