...
**Type :** cadrage produit et économique  
**Portée :** prix d’entrée, services inclus, voix, achats cosmétiques et contraintes de coûts  
**Statut :** modèle économique initial de travail ; certains tarifs secondaires restent à valider par des mesures réelles  
**Date :** 11 août 2026
...
- des publicités dans la relation ;
- des jauges d’affection payantes ;
- l’achat de personnalité ou de souvenirs ;
...
> **achat initial raisonnable + expérience fondamentale complète + options payantes principalement liées à des coûts supplémentaires réels ou à la personnalisation cosmétique.**

---

## 2. Prix d’achat initial
...
Le prix devra être adapté dans les autres devises selon les grilles régionales des stores.

### Pourquoi 4,99 €

Ce prix vise plusieurs objectifs :

- rester suffisamment faible pour permettre un achat de curiosité ;
- éviter l’image d’une application gratuite remplie de limitations ;
- signaler dès le départ qu’il s’agit d’un produit premium sans publicité ;
- créer un premier revenu même chez un utilisateur qui n’achète jamais d’option supplémentaire ;
- rester très inférieur aux abonnements mensuels courants de nombreuses applications de compagnon IA.

Ce prix n’est pas destiné à financer à lui seul plusieurs années de calcul LLM pour un utilisateur très actif.

Il s’agit d’un prix d’accès au produit, pas d’une garantie économique que toutes les consommations réseau futures puissent être illimitées sans autre source de revenu.

---

## 3. Ce que l’achat de base doit inclure
...
L’interface ne doit pas donner la sensation qu’une relation est facturée phrase par phrase.
...
- 0,10 $ / million de tokens d’entrée en cache ;
- 6 $ / million de tokens de sortie.

Le coût réel d’un utilisateur dépendra fortement :

- du nombre de conversations ;
- de la longueur du contexte envoyé au LLM ;
...
L’objectif commercial reste cependant :

> **ne pas facturer chaque conversation textuelle dans l’usage normal.**

Avant le lancement commercial, des tests de coût devront déterminer si cet objectif est soutenable tel quel ou s’il nécessite une politique d’usage raisonnable, une optimisation supplémentaire ou une autre source de revenu récurrent.

---

## 5. Conversation vocale

La conversation vocale constitue une fonctionnalité premium pertinente parce qu’elle apporte une expérience supplémentaire tout en générant un coût d’inférence et d’audio nettement plus direct.

Le principe de monétisation retenu est donc :

> **texte inclus dans l’expérience de base ; voix payante.**

### 5.1 Ne pas proposer une voix réellement illimitée au lancement
...
Une première cible raisonnable à tester serait de l’ordre de **120 minutes par mois**.

Des recharges supplémentaires pourront éventuellement être proposées, par exemple :

- petit pack de minutes ;
- pack moyen ;
- pack important.

Les prix exacts des recharges ne sont pas encore validés.

### 5.3 Pourquoi privilégier une formule récurrente

La voix est une source logique de revenu récurrent parce que :
...
Les vêtements et accessoires constituent la deuxième source de revenu complémentaire envisagée.

Ils doivent rester **cosmétiques**.
...
- rendre un descendant « meilleur » ;
- débloquer une supériorité fonctionnelle.

### 6.1 Grille de prix de travail

Fourchettes initiales recommandées :

- petit accessoire : **0,99 €** ;
- vêtement simple : **1,99 €** ;
- tenue plus travaillée : **2,99 €** ;
- pack ou ensemble thématique : **4,99 €**.

Ces prix sont des cibles de travail et devront être testés auprès des utilisateurs.

### 6.2 Pas de loot boxes

Les achats doivent être explicites.
...
## 7. Ce qui ne doit pas être monétisé pour l’instant

Par défaut, les éléments suivants ne doivent pas devenir des achats séparés :
...
- confiance ;
- vieillissement ;
- ralentissement artificiel du vieillissement cognitif ;
- résurrection après une fin de vie ;
- effacement payant d’un événement gênant ;
- accélération du passage bébé → enfant → adolescent → adulte ;
- avantages héréditaires achetés pour un descendant.

Ces pratiques entreraient directement en conflit avec la persistance individuelle et avec le principe selon lequel l’identité doit venir de l’histoire vécue.

La reproduction et les fonctionnalités intergénérationnelles ne sont pas encore classées comme gratuites ou payantes : cette question reste ouverte tant que le système lui-même n’est pas suffisamment défini.
...
Cette hypothèse n’est pas un calcul comptable définitif et devra être réévaluée avant commercialisation.
...
- Kindroid affiche un abonnement mensuel autour de 15,99 $ sur les stores américains, avec des fonctionnalités audio et autres capacités premium ;
- l’App Store français affiche plusieurs niveaux d’abonnement Replika, dont des offres mensuelles autour de 8,99 € à 15,49 € selon la formule.
...
## 11. Risque économique principal

Le risque principal est un utilisateur qui paie une seule fois 4,99 €, utilise intensivement le texte pendant plusieurs années et n’achète jamais d’option complémentaire.

Si le coût LLM cumulé dépasse largement la marge créée par l’achat initial, cet utilisateur devient structurellement déficitaire.
...
- part de contexte mise en cache ;
- longueur moyenne des échanges ;
- coût des traitements de mémoire et d’évolution hors conversation ;
- taux de conversion vers la voix ;
- revenu moyen des achats cosmétiques ;
- marge réelle après store et fiscalité.
...
- monétisation ou non des fonctions de reproduction et de lignée ;
- prix définitifs après tests utilisateurs et mesure réelle des coûts.