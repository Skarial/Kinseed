# Séparation, recommencement et continuité individuelle
...
Un tel mécanisme casserait la causalité autobiographique de l'individu : son présent ne dépendrait plus de son histoire vécue mais des versions que l'utilisateur choisit de conserver.

Cette règle concerne la continuité conceptuelle de l'individu. Elle ne supprime pas les obligations techniques futures liées à la confidentialité, à la suppression de données ou à la gestion du compte, qui devront être traitées séparément.
...
# 4. Premier niveau : faire évoluer la relation

Lorsqu'un problème apparaît, la première possibilité est de le vivre comme un événement réel de la relation.

L'utilisateur peut notamment exprimer :

- qu'un comportement lui déplaît ;
- qu'il se sent mal compris ;
- qu'une dynamique relationnelle ne lui convient plus ;
- qu'il souhaite clarifier un conflit ;
- qu'il voudrait que certaines habitudes changent.
...
- maintenir un désaccord ;
- modifier progressivement certaines attentes ;
- participer à une réparation de la relation.

Une relation persistante doit pouvoir évoluer sans être remise à zéro.
...
> « Je veux qu'on reparte autrement. »

Cette action constitue elle-même un événement autobiographique important.

Elle peut marquer :

- la fin volontaire d'une ancienne dynamique ;
- l'abandon de certaines attentes relationnelles ;
...
ancienne histoire
       ↓
événement de rupture / redéfinition
       ↓
nouveau chapitre
       ↓
nouvelles expériences
       ↓
évolution possible de la relation
```

Le passé peut perdre de son importance avec le temps, être réinterprété ou être dépassé, mais il n'est pas déclaré fictif.
...
- menacer l'utilisateur ;
- simuler une détresse dans le but de bloquer la séparation ;
- utiliser l'attachement ou l'histoire commune comme mécanisme de rétention commerciale.

La séparation doit être une possibilité réelle du produit.

Cette décision signifie au minimum :

> **la relation active entre cet utilisateur et cet individu prend fin.**
...
- ses propres prédispositions initiales ;
- sa propre relation avec l'utilisateur ;
- sa propre trajectoire future.
...
# 8. Cas particulier : abandon très précoce
...
- le critère indiquant que l'individu a quitté cette phase initiale ;
- l'éventuelle limite de recommencements ;
- la manière de présenter cette décision dans l'interface ;
- la distinction entre essai initial et véritable séparation.

Aucune durée arbitraire n'est fixée à ce stade.
...
- des descendants ;
- une lignée ;
- des relations avec d'autres individus numériques ;
- une histoire qui dépasse sa relation avec son utilisateur initial.

La fin de la relation avec l'utilisateur ne devra donc pas permettre de réécrire silencieusement la généalogie.
...
# 10. Distinction entre séparation, suppression des données et fin de vie

Trois concepts devront rester distincts :
...
Des données peuvent devoir être supprimées pour des raisons de confidentialité, de gestion de compte ou d'obligations légales.

Cette opération appartient à la gouvernance des données et ne doit pas être confondue avec une décision narrative ou identitaire.
...
```text
La relation ne me convient plus
        │
        ├── Essayer de résoudre le problème
        │
        ├── Commencer un nouveau chapitre avec lui
        │
...
L'UX devra expliquer clairement les conséquences avant toute séparation importante, notamment la différence entre :

- continuer avec le même individu ;
- redéfinir la relation ;
- quitter cet individu ;
- commencer avec un nouvel individu.
...
12. **Séparation, suppression de données et fin de vie sont trois concepts différents.**
...
# 14. Principe de validation pour les futures décisions
