...
| D-002 | Héritage | La transmission intergénérationnelle doit permettre combinaison, transformation, variation et oubli, jamais une simple copie. | VALIDÉE | `docs/15-regles-fondatrices-heritage.md` |
...
| D-019 | G0-A3 / identité Memory | Les versions d’une même mémoire épisodique partagent une clé logique stable dérivée de l’identité du Lenoseed et de l’épisode ; le gist reste hors de cette identité, et `revision_of` relie directement les versions successives. | EXPÉRIMENTALE | `docs/13-generation-0a3-protocole-memoire-episodique-minimale.md`, `docs/14-generation-0a3-contrat-implementation.md` |
