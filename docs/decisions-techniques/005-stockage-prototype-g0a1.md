...
témoignage
→ EvidenceItem
→ croyance persistante dans l'état Kinseed
→ correction
→ révision
→ historique conservé
```
...
La remise à zéro du contexte LLM ne nécessite pas que le processus applicatif soit redémarré. Elle vérifie que le modèle ne reçoit plus l’ancien historique conversationnel brut et doit utiliser l’état que Kinseed lui fournit.
...
Cette décision ne transforme pas Kinseed en architecture cloud-first.

`InMemoryStore` est uniquement un **adaptateur expérimental de test**.

L’architecture produit reste :

> **l’état essentiel d’un Kinseed doit exister localement et ne pas dépendre en permanence d’un service distant.**
...
- séparation nette entre logique Kinseed et infrastructure ;
...
> **Le stockage réel doit s’adapter aux invariants du domaine Kinseed ; les invariants du domaine ne doivent pas être simplifiés pour s’adapter prématurément à un stockage choisi trop tôt.**
