...
- **Statut :** accepté
- **Date :** 2026-08-10
- **Périmètre :** application Kinseed
- **Décision :** TypeScript + HTML/CSS + Capacitor

## 1. Contexte

Kinseed a vocation à devenir une application mobile distribuable au grand public.
...
- le cœur métier de Kinseed ;
- l'interface utilisateur ;
- le stockage local ;
- les éventuels services distants ;
- les fonctions propres à Android ou iOS.

Le choix technique doit donc privilégier une base de code commune, une complexité raisonnable et une séparation nette entre la logique de Kinseed et la couche mobile.
...
Capacitor ne doit pas devenir le cœur de l'architecture de Kinseed.
...
Interface HTML / CSS / TypeScript
    │
Cœur métier Kinseed en TypeScript
    │
...
Kinseed doit manipuler de nombreuses entités fortement liées entre elles. Le typage statique permettra notamment de :
...
Lorsqu'une fonction native spécifique est nécessaire, du code Kotlin ou Swift pourra être ajouté localement sans déplacer le cœur métier de Kinseed vers ces langages.
...
Cette option pourra être réévaluée si Kinseed développe plus tard des exigences graphiques ou natives que la stack retenue gère mal.
...
- un typage utile pour les structures complexes de Kinseed ;
...
- une fonction essentielle de Kinseed ne peut pas être correctement réalisée avec Capacitor ;
