...
export function planG0A2SelfHypothesisRevision(input: { readonly kinseedId: EntityId; readonly consolidationId: string; readonly v2: SelfHypothesis; readonly observations: readonly G0A2RevisionObservation[]; }): G0A2RevisionPlan {
  const { v2 } = input;
  if (v2.kinseedId !== input.kinseedId || v2.version !== 2 || v2.status !== "disputed" || v2.confidence !== "low" || v2.stage !== "hypothesis") throw new DomainInvariantError("G0-A2 revision requires disputed v2");
  ...
  const v3Id = buildRevisedG0A2SelfHypothesisId(input.kinseedId, input.consolidationId);
  ...
  const next: SelfHypothesis = { id: v3Id, kinseedId: input.kinseedId, hypothesisKey: v2.hypothesisKey, version: 3, proposition: candidate, stage: "hypothesis", supportLinkIds: links.filter((l) => l.relation === "supports").map((l) => l.id), againstLinkIds: links.filter((l) => l.relation === "contradicts").map((l) => l.id), confidence: "moderate", status: "active", previousVersionId: v2.id, createdAt: timestamp, updatedAt: timestamp };
...
export function buildRevisedG0A2SelfHypothesisId(kinseedId: EntityId, consolidationId: string): EntityId { return `SH-G0A2-${kinseedId}-${consolidationId}-v3`; }
